  ...
  //  econ.startNotifChecker(sock);
    ...
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            // Human edits arrive as a protocol message in messages.upsert,
            // while edits generated by our own send/edit helpers can arrive as
            // messages.update. Feed both forms to the same anti-edit module.
            const editProtocol = (() => {
                const candidates = [
                    mek.message?.protocolMessage,
                    mek.message?.botForwardedMessage?.message?.protocolMessage,
                    mek.message?.message?.protocolMessage,
                    mek.message?.message?.botForwardedMessage?.message?.protocolMessage
                ];
                return candidates.find(protocol => protocol?.editedMessage) || null;
            })();
            if (editProtocol?.editedMessage) {
                try {
                    const antiedit = require('./src/Commands/Tools/antiedit.js');
                    if (antiedit?.onEdit) {
                        await antiedit.onEdit(sock, [{
                            key: editProtocol.key || mek.key,
                            update: { message: editProtocol.editedMessage }
                        }], customStore);
                    }
                } catch (err) {
...
try {
    ...
    if (handled) return;
            ...
            try {
                ...
                if (!_plogmeHandled.has(messageId)) {
                    const plogmeHandled = await plogmeCmd.execute(sock, m, {
                        reply: async (txt) => {
                            try { await sock.sendMessage(m.chat, { text: String(txt || '') + MARKER }, { quoted: m }); } catch (e) {}
                        },
                    ...
                    if (plogmeHandled) {
                        _plogmeHandled.add(messageId);
                        return; // short-circuit other handlers
                    }
                }
            } catch (err) {
    ...
    const normalizeEditUpdates = updates => (Array.isArray(updates) ? updates : []).map(entry => {
        ...
        const protocol = update?.protocolMessage
            ...
            || update?.message?.botForwardedMessage?.message?.protocolMessage
            || entry?.message?.protocolMessage
            || entry?.message?.botForwardedMessage?.message?.protocolMessage;
    ...
    // ========== AUTO-CLEANUP EXPIRED MESSAGES (48 HOUR TTL) ==========
    setInterval(() => {
        const now = Date.now();
        let cleanedCount = 0;
        if (customStore.messages) {
            for (const [key, value] of customStore.messages.entries()) {
                if (value.timestamp && (now - value.timestamp) > MESSAGE_TTL) {
                    ...
                    // For old messages without timestamp, remove them
                    customStore.messages.delete(key);
                    cleanedCount++;
                }
            }
