...
/**
 * Deep-search an object for a value by trying many possible key paths.
 */
...
function formatBanResult(result, requestedNumber = '') {
    // ── Debug: dump the raw API response so we can see what fields it actually returns ──
    try { console.log('[BANCHECK RAW]', JSON.stringify(result, null, 2)); } catch {}

    const number = result?.number || requestedNumber || 'Not provided';
    const banned = Boolean(result?.banned || result?.status === 'banned');

    // ── Reason: try flat + nested paths ──
    const reason = deepFind(result,
        'reason', 'ban_reason', 'banReason', 'banInfo.reason',
        'data.reason', 'data.ban_reason', 'data.banReason',
        'info.reason', 'detail.reason', 'message'
    ) || 'N/A';

    // ── Permanent: try many paths ──
    const permaRaw = deepFind(result,
        'permanent', 'perma', 'isPermanent', 'banInfo.permanent',
        'data.permanent', 'data.perma', 'data.isPermanent',
        'info.permanent', 'detail.permanent'
    );
    const perma = permaRaw === true || permaRaw === 'yes' || permaRaw === 1 ? 'yes' : 'no';

    // ── Banned at: try many paths ──
    const bannedAtRaw = deepFind(result,
        'bannedAt', 'banned_at', 'createdAt', 'created_at', 'banDate', 'ban_date',
        'banInfo.bannedAt', 'banInfo.createdAt', 'banInfo.banDate',
        'data.bannedAt', 'data.banned_at', 'data.createdAt', 'data.created_at',
        'data.banInfo.bannedAt', 'data.banInfo.createdAt',
        'info.bannedAt', 'detail.bannedAt', 'timestamp'
    );
    const bannedAt = formatDateTime(bannedAtRaw);

    // ── Review required at: try many paths ──
    const reviewAtRaw = deepFind(result,
        'reviewRequiredAt', 'review_req_at', 'reviewAt', 'review_date', 'reviewDate',
        'banInfo.reviewRequiredAt', 'banInfo.reviewAt',
        'data.reviewRequiredAt', 'data.review_req_at', 'data.reviewAt',
        'data.banInfo.reviewRequiredAt', 'data.banInfo.reviewAt',
        'info.reviewRequiredAt', 'detail.reviewRequiredAt'
    );
    const reviewAt = formatDateTime(reviewAtRaw);
